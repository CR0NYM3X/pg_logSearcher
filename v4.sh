...
if [ "$#" -lt 2 ] || [ "$#" -gt 4 ]; then
    echo ""
    echo "Uso: $0 /ruta/a/logs \"usuario1,usuario2,usuario3\" [num_cpus] [--sin-detalle]"
    echo ""
    echo "  Argumentos:"
    ...
    echo "    4) --sin-detalle    → (Opcional) No guardar los .log con cada registro encontrado."
    echo "                          Ahorra espacio en disco; se mantienen IPs, tracking y reporte."
    echo ""
    echo "  Ejemplos:"
    echo "    $0 /var/log/postgresql \"postgres,appuser\""
    echo "    $0 /var/log/postgresql \"postgres,appuser\" 4"
    echo "    $0 /var/log/postgresql \"postgres,appuser\" 4 --sin-detalle"
    echo "    $0 /var/log/postgresql \"postgres,appuser\" \"\" --sin-detalle"
    echo ""
    exit 1
fi
...
# ── Manejar --sin-detalle en posición 3 (sin num_cpus) ───────────────────────
if [ "$CPU_ARG" = "--sin-detalle" ]; then
...
if [ -n "$SIN_DETALLE" ] && [ "$SIN_DETALLE" != "--sin-detalle" ]; then
    echo "ERROR: Argumento no reconocido: '$SIN_DETALLE'. ¿Quisiste usar --sin-detalle?"
    exit 1
fi

# ── Flag de guardado de detalle ───────────────────────────────────────────────
GUARDAR_DETALLE=1
if [ "$SIN_DETALLE" = "--sin-detalle" ]; then
    GUARDAR_DETALLE=0
fi
...
if [ "$GUARDAR_DETALLE" -eq 0 ]; then
    echo "  Detalle de logs : DESHABILITADO (--sin-detalle)"
else
    echo "  Detalle de logs : habilitado"
fi
echo "--------------------------------------------------"
...
procesar_archivo() {
    ...
    for usuario in "${lista_usuarios[@]}"; do
        ...
        # ── Determinar destino de las líneas de log ──────────────────────────
        # Con --sin-detalle: se descartan las líneas (solo se extraen IPs)
        # Sin la flag: se guardan en el .log habitual
        local destino_log
        if [ "$guardar_detalle" -eq 1 ]; then
            destino_log="$RESULT_BASE_DIR/$usuario/${nombre_base}.log"
        else
            destino_log="/dev/null"
        fi
        ...
        # Con --sin-detalle no existe el .log, así que verificamos por las IPs
        # o por el exit code / contenido del grep previo.
        if [ "$guardar_detalle" -eq 1 ]; then
            ...
            # Sin detalle: usamos las IPs extraídas como indicador de actividad
            if [ -s "$destino_ips" ]; then
                echo "[$ts] Archivo: $nombre_gz -> ENCONTRADO" \
                    >> "$RESULT_BASE_DIR/$usuario/seguimiento_revision.txt"
            else
                echo "[$ts] Archivo: $nombre_gz -> VACÍO" \
...
for usuario in "${lista_usuarios[@]}"; do
    TRACKING_FILE="$RESULT_BASE_DIR/$usuario/seguimiento_revision.txt"
    {
        ...
        if [ "$GUARDAR_DETALLE" -eq 0 ]; then
            echo "Modo: --sin-detalle (los .log de registros individuales NO se guardaron)"
        fi
        echo "-------------------------------------------"
    } > "$TRACKING_FILE"
...

{
    ...
    if [ ${#usuarios_con_hits[@]} -eq 0 ]; then
        ...
        for usuario in "${usuarios_con_hits[@]}"; do
            ...
            # Solo disponible cuando se guardan los .log (sin --sin-detalle)
            conn_count=0
            stmt_count=0
            if [ "$GUARDAR_DETALLE" -eq 1 ]; then
            ...
            if [ "$GUARDAR_DETALLE" -eq 1 ]; then
                ...
                echo "  │  (desglose CONN/STMT no disponible en modo --sin-detalle)"
            fi
            echo "  │"
...
echo "  Resultados en: $RESULT_BASE_DIR"
if [ "$GUARDAR_DETALLE" -eq 0 ]; then
    echo "  Modo: --sin-detalle (no se guardaron .log individuales)"
fi
echo ""
for usuario in "${lista_usuarios[@]}"; do
    TRACKING_FILE="$RESULT_BASE_DIR/$usuario/seguimiento_revision.txt"
